    ...
    for (let i = 0; i < intersectedBarcodes.length; i++)
      for (let j = 0; j < expressionData.length; j++)
        if (expressionData[j].tcga_participant_barcode == intersectedBarcodes[i])
          expData.push(expressionData[j])
    for (let j = 0; j < expressionData.length; j++)
      if (expressionData[j].tcga_participant_barcode == intersectedBarcodes[i])
        expData.push(expressionData[j])
    expData = expData.filter(x => clickedGenes.includes(x.gene))

    // call firebrowse.getClinical_FH function (using given barcodes)
    let clinData = await firebrowse.getClinical_FH(intersectedBarcodes);

    buildHeatmap(expData, clinData);
    buildViolinPlot(cohortQuery, expData);
