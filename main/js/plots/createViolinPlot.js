...
const createViolinPlot = async function(dataInput, violinDiv, curPlot, facetByFields) {
  ...
  const divNum = violinDiv.id[violinDiv.id.length - 1];
  let clinicalData = "";
  if (facetByFields.length > 0) clinicalData = JSON.parse(localStorage.getItem("clinicalData"));
  ...
  if (facetByFields.length > 0) {
    for (let i = 0; i < dataInput.length; i++) {
      ...
      const clinicalDataIndex = findMatchByTCGABarcode(dataInput[i], clinicalData);
      if (clinicalDataIndex >= 0) {
        //Add parentheses for formatting purposes
        let keyToFacetBy = `${dataInput[i]["cohort"]  } (`;
        //Iterate over the clinical fields to facet by to create keyToFacetBy
        for (let fieldIndex = 0; fieldIndex < facetByFields.length; fieldIndex++) {
          //Append additional JSON field to data for the purpose of creating a key to facet by
          const clinicalField = facetByFields[fieldIndex];
    ...
    myGroups = d3.map(dataInput, function(d) {
  ...
  function compareGeneExpressionMedian(a, b) {
    const aArray = d3.map(dataInput.filter(x => x.cohort === a), function(d) {
      return d.expression_log2;
    }).keys();
    const bArray = d3.map(dataInput.filter(x => x.cohort === b), function(d) {
      return d.expression_log2;
    }).keys();
    const aMedian = d3.quantile(aArray.sort(function(a, b) {
      return a - b;
    }), 0.5);
    const bMedian = d3.quantile(bArray.sort(function(a, b) {
      return a - b;
    }), 0.5);
  ...
  //.attr("viewBox", `0 -50 1250 475`)  // This line makes the svg responsive
    ...
    .attr("id", svgID).attr("indepVarType",
      "gene") //The attributes added on this line and the lines below are used when rebuilding the plot
    .attr("cohort", curPlot).append("g").attr("id", (`${svgID  }Position`)).attr("transform", `translate(${  margin.left  },${
  ...
  const geneExpressionValues = d3.map(dataInput, function(d) {
    return d.expression_log2;
  }).keys();
  const minExpressionLevel = Math.min(...geneExpressionValues);
  const maxExpressionLevel = Math.max(...geneExpressionValues);
  // Build and Show the Y scale
  const y = d3.scaleLinear().domain([minExpressionLevel,
    maxExpressionLevel
  ]).range([height, 0]);
  svgObject.append("g").call(d3.axisLeft(y));
  //Append y-axis label
  svgObject.append("text").attr("transform", "rotate(-90)").attr("y", 0 - margin.left).attr("x", 0 - (height / 2))
    .attr("dy", "1em").style("text-anchor", "middle").text("Expression Level");
  // Build and Show the X scale. It is a band scale like for a boxplot: each group has an dedicated RANGE on the axis. This range has a length of x.bandwidth
  const x = d3.scaleBand().range([0, width]).domain(myGroups).padding(
    0.01); // This is important: it is the space between 2 groups. 0 means no padding. 1 is the maximum.
  svgObject.append("g").attr("transform", `translate(0,${  height  })`).call(d3.axisBottom(x)).selectAll(
    ".tick text").attr("transform", "rotate(-45), translate(-10, 5)").call(wrap, x.bandwidth());
  svgObject.append("text").attr("transform", `translate(${  width/2  }, ${  height + margin.top + 30  })`)
  ...
  const sumstat = d3.nest().key(function(d) {
    if (facetByFields.length === 0) {
      return d["cohort"];
    } else {
      return d.facetByFieldKey;
  ...
  for (const i in sumstat) {
    ...
    if (facetByFields.length === 0) {
      currentExpressionArray = d3.map(dataInput.filter(x => x["cohort"] === sumstat[i].key), function(d) {
        return d.expression_log2;
      }).keys();
    } else {
      currentExpressionArray = d3.map(dataInput.filter(x => x.facetByFieldKey === sumstat[i].key), function(d) {
        return d.expression_log2;
      }).keys();
    }
    currentExpressionArray.sort(function(a, b) {
      return a - b;
    });
  ...
  // create a tooltip
  const tooltip = d3.select(`#${  svgDivId}`).append("div").style("opacity", 0).attr("id", `tooltip${  divNum}`)
    .attr("class", "tooltip").style("background-color", "white").style("border", "solid").style("border-width",
      "2px").style("border-radius", "5px").style("padding", "5px");
  ...
  const mousemove = function(d) {
    tooltip.style("left", `${d3.mouse(this)[0]+70  }px`).style("top", `${d3.mouse(this)[1]  }px`).attr(
      "transform", `translate(${  width/4  })`);
    for (const prop in this) {
      const spacing = "\xa0\xa0\xa0\xa0|\xa0\xa0\xa0\xa0";
      const tooltipstring = `${"\xa0\xa0" +
  ...
  const xNum = d3.scaleLinear().range([0, x.bandwidth()]).domain([-maxNum,
    maxNum
  ]);
  ...
  svgObject.selectAll("myViolin").data(sumstat).enter() // So now we are working group per group
    .append("g").attr("transform", function(d) {
      xVals.push(x(d.key) + (x.bandwidth() / 2));
      return (`translate(${  x(d.key) } , 0)`);
    }) // Translation on the right to be at the group position
    .on("mouseover", mouseover).on("mousemove", mousemove).on("mouseleave", mouseleave).append("path").datum(
      function(d) {
        return (d.value);
      }) // So now we are working bin per bin
    .style("stroke", "none").style("fill", function(d) {
      const colorToReturn = violinCurveColors[colorsIndex];
      colorsIndex++;
      return colorToReturn;
    }).attr("d", d3.area().x0(function(d) {
      return (xNum(-d[1]));
    }).x1(function(d) {
      return (xNum(d[1]));
    }).y(function(d) {
      ...
      .curveCatmullRom)); // This makes the line smoother to give the violin appearance. Try d3.curveStep to see the difference
  //Embed box-and-whisker plot inside of each violin curve
  for (let index = 0; index < sumstat.length; index++) {
    //Adding whisker on the box-and-whisker plot
    svgObject.append("line").attr("x1", xVals[index]).attr("x2", xVals[index]).attr("y1", y(sumstat[index].min))
      .attr("y2", y(sumstat[index].max)).attr("stroke", "black").attr("stroke-width", x.bandwidth() / 500);
    //Adding rectangle for each box-and-whisker plot
    const rectWidth = x.bandwidth() / 25;
    svgObject.append("rect").attr("x", xVals[index] - rectWidth / 2).attr("y", y(sumstat[index].Qthree)).attr(
      "height", y(sumstat[index].Qone) - y(sumstat[index].Qthree)).attr("width", rectWidth).attr("stroke",
      "black").style("stroke-width", x.bandwidth() / 500).attr("fill", "none");
    //Median line for box-and-whisker plot
    svgObject.append("line").attr("x1", xVals[index] - rectWidth / 2).attr("x2", xVals[index] + rectWidth / 2).attr(
      "y1", y(sumstat[index].median)).attr("y2", y(sumstat[index].median)).attr("stroke", "black").attr(
      "stroke-width", x.bandwidth() / 500);
  }
  // Add title to graph
  svgObject.append("text").attr("x", 0).attr("y", -25).attr("text-anchor", "left").style("font-size", "26px").text(
    `Gene Expression Violin Plot for ${ curPlot}`);
};
...
const createViolinPartitionBox = async function(violinsDivId, geneQuery) {
  const partitionDivId = "violinPartition";
  const div_box = d3.select(`#${partitionDivId}`);
  div_box.append("text").style("font-size", "20px").text("Select variables to partition violin curves by:");
  div_box.append("div").attr("class", "viewport").attr("id", "partitionSelectViolinPlot").style("overflow-y",
    "scroll").style("height", "90px").style("width", "500px").append("div").attr("class", "body");
  const selectedText = div_box.append("text");
  const div_body = div_box.select(".body");
  const choices = [];
  ...
  function update() {
    ...
    if (choices.length > 0) {
      selectedText.text(`Selected: ${  choices.join(", ")}`);
    } else {
  ...
  function renderCB(div_obj, data) {
    ...
    label.append("label").attr("class", "switch").append("input").attr("class", "myViolinCheckbox").attr("value",
      data).attr("type", "checkbox").on("change", function() {
      update();
      rebuildViolinPlot(partitionDivId, geneQuery);
    }).attr("style", "opacity: 1; position: relative; pointer-events: all").append("span").attr("class",
      "slider round").attr("value", data);
    label.append("text").text(data);
  }
  // data to input = clinical vars from query
  const clinicalVars = localStorage.getItem("clinicalFeatureKeys").split(",");
  const var_opts = clinicalVars;
...
const getPartitionBoxSelections = function(violinsDivId) {
  const selectedOptions = [];
  d3.select(`#${violinsDivId}`).selectAll(".myViolinCheckbox").each(function(d) {
    const cb = d3.select(this);
    if (cb.property("checked")) {
      selectedOptions.push(cb.property("value"));
    }
  });
  return selectedOptions;
};
  //Rebuilds the violin plot associated with violinDivId
const rebuildViolinPlot = function(violinsDivId, geneQuery) {
  const selectedOptions = getPartitionBoxSelections(violinsDivId);
  //geneQuery = geneQuery.split(",");
  for (let index = 0; index < geneQuery.length; index++) {
    const svgDivId = `svgViolin${  index}`;
    const svgDiv = document.getElementById(svgDivId);
    svgDiv.innerHTML = "";
    const violinDivId = `violinPlot${  index}`;
    createViolinPlot(JSON.parse(localStorage.getItem("expressionData")), document.getElementById(violinDivId),
      geneQuery[index], selectedOptions);
  }
};
...
function findMatchByTCGABarcode(patient, clinicalData) {
  for (let index = 0; index < clinicalData.length; index++) {
    if (clinicalData[index]["tcga_participant_barcode"] == (patient["tcga_participant_barcode"])) return index;
  }
  return -1;
}
