...
let buildDataExplorePlots = async function(metadata) {
    ...
    if($('.cancerTypeMultipleSelection').select2('data').map(cancer => cancer.text).length > 0) {
        ...
        // get selected genes and clinical features
        ...
        // if no genes or clinical features are selected, do not display any pie charts
        if(mySelectedClinicalFeatures.length == 0) {
            document.getElementById('dataexploration').innerHTML = ""

        // if genes or clinical feature(s) are selected, display appropriate pie chart(s)
        } else {

            // clear all previous plots that were displayed
            document.getElementById('dataexploration').innerHTML = "";

            // loop through each selected item (i.e., gene or clinical feature)
            for(let i = 0; i < mySelectedClinicalFeatures.length; i++) {
                ...
                // get values and labels for this feature
                    ...
                    for(let i = 0; i < metadata.length; i++) 
                        allValuesForCurrentFeature.push(metadata[i][currentFeature]);
