...
def main():
    source = '/data01/shared/skobayashi/webgen/PAAD_AE/scaled_400/'
    ...
    for i in range(len(output)):
        ...
        saveName=str(fn[i])[:-4] + '_reConstructed_50Epoch_noNorm_Check.png'
        im.save(saveName)
        
        print(images[i])
        orig =transforms.ToPILImage()(images[i]).convert("RGB")
        orig_saveName=str(fn[i])[:-4] + '_orig_50Epoch_noNorm_Check.png'
        orig.save(orig_saveName)
