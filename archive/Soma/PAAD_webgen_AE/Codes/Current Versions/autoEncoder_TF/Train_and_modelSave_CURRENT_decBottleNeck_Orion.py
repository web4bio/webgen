...
class CVAE(tf.keras.Model):
  ...
  @tf.function(input_signature=[tf.TensorSpec(shape=(None,400,400,3), dtype=tf.float32)])
  def encode(self,X):
...
def main():
    ...
    outputs_dir = os.path.join(os.getcwd(),'outputs_savedModelTest')
    ...
    source = '/home/skobayashi/webgen_PAAD_400x400/Split/'
    ...
    img_trains = img_trains[:10]
    img_test = img_test[:10]
    ...
    # checkpoint
    filepath="./saved_models/weights_best.h5"
    ...
    print(pred_autoencoder.signatures["serving_default"])
    for fn, testImg in test_imgs_dict.items():
        print('ENCODE????')
        test = pred_autoencoder.encode(testImg)
        print(test.shape)
